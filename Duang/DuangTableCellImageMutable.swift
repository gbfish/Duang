...
class DuangTableCellImageMutable: UITableViewCell
{
    ...
    var photos: PFRelation? {
        didSet {
            ...
            if let relation = photos {
                if let theRelationQuery = relation.query() {
                    theRelationQuery.findObjectsInBackgroundWithBlock({ (objects, error) -> Void in
                        if error == nil {
                            if let photos = objects as? [PFObject] {
                                switch photos.count {
                                case 1:
    ...
    func getFile(photos: [PFObject], index: NSInteger) -> PFFile? {
        ...
        if let returnValue = photo[TablePhoto.Image] as? PFFile {
            return returnValue
        }
        return nil
    }
